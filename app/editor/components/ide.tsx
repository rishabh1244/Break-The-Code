...
export default function Ide({ fileName, metadata, code, setCode }: IdeProps) {
  ...
  const RunCode = async () => {
    ...
    try {
      ...
      // const res = await fetch(`${process.env.COMPILE_SERVER}?slug=${(metadata.name as any).replace(/ /g, "_")}`, {
      ...
      if (!res.ok) {
        ...
        setResult({ compiled: false, error: errorData.error || `Request failed with status ${res.status}` });
        setShowOutput(true);
        setIsRunning(false);
        return;
      }
      ...
      // The API returns { success: true, message: output } where output is the CompileResult
      setResult(data.message || data);
      setShowOutput(true);
    } catch (error: any) {
      console.error("Error running code:", error);
      setResult({ compiled: false, error: error.message || "Failed to run code. Please try again." });
      setShowOutput(true);
    } finally {
      setIsRunning(false);
    }
